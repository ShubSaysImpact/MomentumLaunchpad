...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const toggleGoal = (id: string) => {
    setData(prev => ({
      ...
      goals: prev.goals.map(g =>
        g.id === id ? { ...g, completed: !g.completed, completedAt: !g.completed ? new Date().toISOString() : undefined } : g
      ),
    }));
  };
  ...
  const addResonanceTest = (category: keyof ResonanceTestData, test: Omit<ResonanceTest, 'id' | 'createdAt'>) => {
    ...
    setData(prev => {
        const prevResonanceTests = prev.resonanceTests ?? { founderStory: [], ideaPitch: [], offer: [] };
        return {
            ...prev,
            resonanceTests: {
                ...prevResonanceTests,
                [category]: [...(prevResonanceTests[category] ?? []), newTest]
            }
        };
    });
  };
